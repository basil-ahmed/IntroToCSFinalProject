...
import os
player = Minim(this)
class Game: #The class that includes all the game features
    def __init__(self, w, h, g, gr):
        ...
        self.level = 0
        self.kid = Player(self.g+100, self.g+100, self.g, 800, 100, 100) #Calling the player class in game for the character functionality
        self.platforms = []
    ...
    def form_platform(self): #Randomly forms different types of platfrorms
        if self.cnt == 8:
            for i in range(8):
        ...
        for p in self.platforms: #remove platforms when they reach the top
            if p.y == 0:
        ...
        if self.game_over == False and self.cnt == 1: #Will stop forming platforms when game ends
            self.cnt = self.cnt - 1
            position = random.randint(0,4)
            type = random.randint(0,4)
            self.platforms.append(Platform(120,25,position*120,800,type))       
                
    def display(self): # displaying the spike row on top of the screen
...
class Player: #includes character's functionality
    ...
    def gravity(self): #Falling of the character
    ...
    def update(self):
        ...
        if 800 - self.char_h == self.y and game.game_over == False: #Conditions for Game Over
            self.game_over()
...
class Platform: #Forms platforms
  ...
  def move(self): #moving platforms upwards
      self.y = self.y - 4 - (game.level/100) #increasing speed as game moves on
  
  def display(self):
    image(self.img,self.x,self.y,self.w,self.h)

game = Game(600,800,51,800) #Initiating game class
...
def draw():
    ...
    fill(255,255,255)
    textSize(40)
    text('Score:'+str((game.level//50)+1),10,80) #Increase 1 score for 50 platforms passed
